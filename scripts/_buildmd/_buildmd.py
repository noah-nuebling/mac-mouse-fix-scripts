...
def main():
    ...
    for document_key in document_keys:
        ...
        for locale in iterated_locales:
            ...
            elif document_key == "Support/Guides/CapturedButtonsMMF3.md": 
                template = insert_capture_guide_screenshots(template, destination_path, locale, development_locale, translation_progress)
            else:
...
def insert_capture_guide_screenshots(template, destination_path: str, preferred_locale: str, source_locale: str, translation_progress: dict):

    # Negotiate locale
    #   - Keep in-sync with `testTakeScreenshots_Documentation()`, which generates these screenshots  [Dec 2025]
    #       (There we just use `-[NSBundle localizations]`)
    #   - Also see usage of `babel.negotiate_locale()` [Dec 2025]
    locale = (
        preferred_locale 
        if (
            (preferred_locale == source_locale) or
            (translation_progress[preferred_locale]['percentage'] > 0)
        ) 
        else source_locale
    )

    # Construct image paths
    repo_root = mflocales.mainmdp_path_to_repo_root(destination_path)
    src0 = f'{repo_root}/Markdown/Media/Autogenerated/{locale}/CapturedButtons0.jpg">'
    src1 = f'{repo_root}/Markdown/Media/Autogenerated/{locale}/CapturedButtons1.jpg">'
    src2 = f'{repo_root}/Markdown/Media/Autogenerated/{locale}/CapturedButtons2.jpg">'

    # Validate image paths
    for src in [src0, src1, src2]:
        assert Path(destination_path + src).exists, f"Inserting image with path '{src}' into '{destination_path}', but the image doesn't exist."

    # Insert into template
    template = template.replace('{screenshot_0}', f'<img height=200 src="{src0}"')
    template = template.replace('{screenshot_1}', f'<img width=600  src="{src1}"')
    template = template.replace('{screenshot_2}', f'<img width=600  src="{src2}"')
...
def display_name(sale):
    ...
    elif name == "🇺🇦 Github Com/Alex Danilenko":                name = "🇺🇦 Alex Danilenko"    # [Aug 2025]
